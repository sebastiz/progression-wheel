...
["acidtechno", "Acid Techno", "The 303 is the arrangement. The kick locks four-on-the-floor in the first bar and never moves again, so every section here is the same loop with the acid line pushed somewhere new — the filter and the resonance do what a layer does in every other style, and the one subtraction is the kick being pulled out from under the squelch rather than the line being stopped.",
 { bpm: 140, drum: "techno16", kit: "909", pump: "hard", pat: "quarters", bass: "acidline", bassVoice: "acid", pad:"glass", percKit:"machine", delay:"16", swing:0, instr:"lead_2_sawtooth", melInstr:"acid", humanise:0, narrative:"ostinato", vary:0.4, sync:0, trackFx:{ bass:{ res:75, drive:45 } } }, [
 ["Intro|LOOP|4|sixteen bars of kick and the 303 alone with the filter nearly shut — the line introduces itself before the record does",
  { chords: 0, parts: "", filter: [0.12, 0.25], res: [0.3, 0.4] }],
 ["Groove|LOOP|8|thirty-two bars: the drums are already everything they will ever be, and the acid filter creeps open across the whole of it",
  { parts: "", filter: [0.25, 0.5], res: [0.4, 0.55] }],
 ["Build|LOOP|4|the resonance bites harder and the line starts to scream — nothing is added here, the 303 is simply pushed",
  { parts: "A", filter: [0.5, 0.7], res: [0.55, 0.8] }],
 ["Peak|LOOP|4|the acid wide open and squealing over a kick that is identical to bar one",
  { parts: "AB", filter: [0.7, 0.95], res: [0.8, 0.9], trans: "slam" }],
 ["Kick out|HALF1|4|eight bars with the floor pulled away and the line left running — in acid techno the drop is the kick leaving, never the 303 stopping",
  { drums: "nokick", parts: "A", res: [0.9, 0.9], trans: "gap1" }],
 ["Rebuild|LOOP|4|the kick lands back underneath and the bass switches to the straight techno roll — the same climb, on a line that turns the bar over",
  { parts: "A", bassPat: "techno", filter: [0.45, 0.8], res: [0.8, 0.85] }],
 ["Peak|LOOP|8|thirty-two bars, everything in at once and the resonance at its most vicious — the longest the acid is ever let run free",
  { parts: "ABC", filter: [0.8, 1], res: [0.9, 1], trans: "revdrop" }],
 ["Outro|LOOP|4|the filter shuts on the 303 first and the kick plays out alone for the mix — the line gets the last word, same as the first",
  { chords: 0, parts: "", filter: [1, 0.1], res: [1, 0.3] }],
]],
...
[
 ...
 ["Techno / Electro / EBM", ["techno", "minimaltechno", "dubtechno", "hardtechno", "acidtechno",
   "industrialtechno", "melotech", "electrofunk", "electro2000s", "ebm", "newbeat"]],
...
/* What a section is worth on the energy staircase, which is the picture the strip draws. The
   ...
   `bass`, `perc` and `pad` used to be left out entirely, which is why a breakdown that drops all
   three — the textbook move — used to read as barely a dip: nothing costed anything except the
